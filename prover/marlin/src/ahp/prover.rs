...
impl<F: PrimeField> AHPForR1CS<F> {
    ...
    pub fn prover_init<C: ConstraintSynthesizer<F>>(
        index: &Index<F>,
        c: C,
    ) -> Result<ProverState<'_, F>, Error> {
        ...
        let result = pcs.is_satisfied().unwrap();
        assert!(result);
    ...
    pub fn prover_third_round<R: RngCore>(
        ver_message: &VerifierSecondMsg<F>,
        prover_state: ProverState<'_, F>,
        _r: &mut R,
